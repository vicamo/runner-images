...
Function GenerateResourcesAndImage {
    <#
        ...
        .PARAMETER ImageSource
            The build source of image. Valid values are: AzureARM.
    ...
    #>
    [CmdletBinding(DefaultParametersetName='AzureARM')]
    param (
        ...
        [ImageSource] $ImageSource = [ImageSource]::AzureARM,
        [Parameter(Mandatory = $False)]
        ...
        [Parameter(Mandatory = $False, ParameterSetName = 'AzureARM')]
    ...
    if ($PSCmdlet.ParameterSetName -ne $ImageSource) {
        throw "Parameters of $($PSCmdlet.ParameterSetName) passed while image source $ImageSource in use. "
    }
    ...
    try {
        if ($ImageSource -eq [ImageSource]::AzureARM) {
